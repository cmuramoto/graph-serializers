...
public class ProvisionService implements AutoCloseable {
	...
	public static final class Builder {
			...
			synchronized (ProvisionService.class) {
				if (INSTANCE != null) {
					throw new IllegalStateException(
							"Provision Service Already Constructed!!!");
				}
			}
		}
		...
		public ProvisionService bootstrap() {
			synchronized (ProvisionService.class) {
				...
				ProvisionService service = new ProvisionService(maxMem,
						onlyAutoMap, configFrozen, skipResources, failOnCNFE);
	...
	public static synchronized ProvisionService getInstance() {
		...
		if (ps == null) {
			ps = new ProvisionService(16 * 1024 * 1024, false, false, false,
					false);
			ps.installMappings();
			INSTANCE = ps;
		}
	...
	static List<URL> locateClasspathResources(ClassLoader cl, Set<String> nss) {
		...
		for (String ns : nss) {
			URL url = cl.getResource(String.format("META-INF/mappings/%s.xml",
					ns));
	...
	static Class<?>[] reserved() {
		return new Class<?>[] { null, boolean.class, char.class, byte.class,
	...
	public ProvisionService(int maxMem, boolean onlyAutoMap,
			boolean configFrozen, boolean skipResources, boolean failOnCNFE) {
		super();
		this.maxMem = maxMem;
	...
	public synchronized void close() {
		...
		if (m != null) {
			...
			Iterator<Entry<Integer, ValidMapping>> itr = m.entrySet()
					.iterator();
			while (itr.hasNext()) {
				itr.next();
				itr.remove();
	...
	/**
	 * <p>
	 * Attempts to evaluate the smallest required prime number required to
	 * ensure a 'perfect hashing' scheme. By 'perfect' we mean we can find a
	 * prime number P, such that {@link Class#hashCode()} % P evaluates maps to
	 * a unique number for every {@link ValidMapping}'s type, that is, we can
	 * normalize the hashes to a small sub-set of numbers.
	 * </p>
	 * 
	 * <p>
	 * This method assumes that all hashCodes generated by
	 * {@link System#identityHashCode(Object)} are positive numbers, which, at
	 * least for <a href=
	 * "http://hg.openjdk.java.net/jdk8/jdk8/hotspot/file/87ee5ee27509/src/share/vm/oops/markOop.hpp"
	 * >hotspot</a> VM, which stores the identity hash either as a 25bit number
	 * (32bit VM) or as a 31bit number (64bit VM), is true.
	 * </p>
	 * 
	 * <p>
	 * Bellow, an excerpt of markOop.hpp. If by any chance the VM changes the
	 * hashing scheme, we'd have to mask the hash with 0x7fffffff in order to
	 ...
	 * 
	 * <p>
	 * If we succeed to find a perfect hash, we can be sure that we can create
	 * the mapping {@link Class}<=>int in a way that lookups can be made
	 * loop/recursion-free. If the required number for the perfect hash is too
	 * big, it might be faster to fall back to loop based lookups due to cache
	 * line effects.
	 * </p>
	 * 
	 * <p>
	 * The association of a type with an index could also have been done with a
	 * power of two approach, which would use a lesser expensive operation (hash
	 * & (P-1)), where P is a power of two. This however, tends to require a
	 * much larger P than with prime approach. For 230 mappings, the power of
	 * two normalization requires about 64K slots, whilst the prime
	 * </p>
	 * 
	 * @param fullMap
	 *            all known types
	...
	private void evaluateTypes(TreeMap<Integer, ValidMapping> fullMap) {
		...
		int p = PrimeFinder.nextPrime(values.size() << 1);
		int sz = p << 2;
		...
		l: while (sz > 0 && sz <= maxMem && p > 0) {
			...
			for (ValidMapping vm : values) {
				...
				if ((old = hashes.put(ix, vm.type)) != null) {
					p = PrimeFinder.nextPrime(p + 1);
					sz = p << 2;
					hashes.clear();
					largest = -1;

					Log.debug(
							"Detected Hash collision: [%s,%d]<=>[%s,%d], expanding to %d",
							vm.type.getName(), ix, old.getName(), ix, p);
		...
		cd = Pair.of(success == null ? null
				: new int[] { success, largest + 1 }, Pair.of(cids, types));
	}

	public Pair<int[], Pair<int[], Class<?>[]>> getClassData() {
		return cd == null ? Pair.<int[], Pair<int[], Class<?>[]>> of(null,
				reservedOnly()) : cd;
	}
	...
	void installMappings() {
		...
		ServiceLoader<NamespaceLocationService> lookups = ServiceLoader
				.load(NamespaceLocationService.class);

		Set<String> nss = new THashSet<>();
	...
	private void installMappings(List<TypeMappings> tms,
			TreeMap<Integer, ValidMapping> fullMap) {
		for (TypeMappings tm : tms) {
			installMappings(tm, fullMap);
		}
	}

	private void installMappings(TypeMappings tm,
			TreeMap<Integer, ValidMapping> fullMap) {
			...
			try {
				...
				if (rt instanceof Class<?>) {
					...
					if (ser != null && m.getSerializerLookup() != null) {
						gs = Class.forName(ser)
								.getDeclaredMethod(m.getSerializerLookup())
								.invoke(null);
					} else {
						gs = ser;
					}
				...
				ValidMapping pair = new ValidMapping(resolvedType,
						resolvedObject, gs, m.getIncludes(), m.isOpaque(),
						m.isSkipReify());
				...
				if (old != null) {
					throw new IllegalMappingException(
							String.format(
									"Type id %d is being shared by types: %s and %s. Mappings must be unique.",
									id, old.type.getName(),
	...
	private void validateReservedMappings(TreeMap<Integer, ValidMapping> fullMap) {
		...
		for (int i = 1; i < types.length; i++) {
			...
			if (type != types[i]) {
				throw new IllegalMappingException(
						String.format(
								"Reserved types MUST be mapped in the range [1,%d]. Offending mapping: %s, id=%d",
								types.length - 1, type, i));
			}
		}
	}
